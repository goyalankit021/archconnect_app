...
class _ArchitectProfileScreenState extends ConsumerState<ArchitectProfileScreen> {
  ...
  Future<void> _handleUpload({
    ...
    try {
      String path = docType == 'profile' ? 'users/$uid/profile.jpg' : 'users/$uid/kyc/$docType.jpg';
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  ref.invalidate(profileBankDetailsProvider);
  ...
  Widget build(BuildContext context) {
    ...
    return userAsync.when(
      data: (userData) {
        ...
        final kycStatus = userData['kycStatus'] ?? 'pending'; // ✅ Get Status
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      Stack(
                        children: [
                          CircleAvatar(
                            ...
                            child: profileUrl == null ? const Icon(Icons.person, size: 50, color: Colors.grey) : null,
                          ),
                          Positioned(
                            bottom: 0,
                            right: 0,
                      ...
                      // Responsiveness: Prevent name overflow
                      Row(
                      ...
                      Text(firmName, style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 14), maxLines: 1, overflow: TextOverflow.ellipsis),
                    ],
                  ),
                ),

                // --- SMART STATUS BANNERS (User-Friendly Logic) ---
                if (kycStatus == 'verification_pending')
                  _buildAlertBanner(Colors.blue, Icons.hourglass_top, "Verification Pending", "We are reviewing your details.")
                else if (kycStatus == 'rejected')
                  _buildAlertBanner(Colors.red, Icons.error_outline, "Action Required", "KYC documents rejected. Please update.")
                else if (!isProfileComplete)
                    _buildAlertBanner(Colors.orange, Icons.warning_amber_rounded, "Profile Incomplete", "Complete setup to unlock features."),

                // (If verified, we show no banner, keeping the UI clean)
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () {
                    Navigator.push(context, MaterialPageRoute(builder: (context) => const BankDetailsForm()));
                  },
  ...
  Widget _buildKycTab(Map<String, dynamic> data, String uid) {
    ...
    if (kycStatus == 'verification_pending') statusColor = Colors.blue;
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                // ✅ FIX: Wrapped text in Expanded to prevent overflow on small screens
                Expanded(
                  child: Text(
                    "Current Status: ${kycStatus.toString().toUpperCase().replaceAll('_', ' ')}",
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // ✅ FIX: Preserved Expanded wrapper to prevent overflow
          Expanded(
            child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
  ...
  Widget _buildUploadCard({required String title, required String subtitle, required bool isUploaded, required VoidCallback onTap}) {
    return Container(
      ...
      builder: (context) {
        return Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Edit Personal Details", style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
              const SizedBox(height: 20),
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: "Full Name", prefixIcon: Icon(Icons.person))),
              const SizedBox(height: 12),
              TextField(controller: firmCtrl, decoration: const InputDecoration(labelText: "Firm Name", prefixIcon: Icon(Icons.business))),
              const SizedBox(height: 12),
              TextField(controller: emailCtrl, keyboardType: TextInputType.emailAddress, decoration: const InputDecoration(labelText: "Email Address", prefixIcon: Icon(Icons.email))),
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(child: TextField(controller: cityCtrl, decoration: const InputDecoration(labelText: "City", prefixIcon: Icon(Icons.location_city)))),
                  const SizedBox(width: 12),
                  Expanded(child: TextField(controller: stateCtrl, decoration: const InputDecoration(labelText: "State", prefixIcon: Icon(Icons.map)))),
                ],
              ),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("We are here to help! Reach out to us via:", textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: Colors.grey)),
            const SizedBox(height: 20),
            _buildContactTile(
              icon: Icons.phone,
              color: Colors.green,
  ...
  Widget _buildContactTile({required IconData icon, required Color color, required String title, required String subtitle, required VoidCallback onTap}) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle), child: Icon(icon, color: color, size: 20)),
            const SizedBox(width: 16),
            // ✅ FIX: Preserved Expanded wrapper
            Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
